    ...
    prompt = f"""
            ...
            테이블 간 관계:
                ...
                **조건**:  
                    ...
                    2. **시간 범위 조건**:  
                    - 질문에 포함된 시간 범위(예: "지난 2일", "지난 1주일", "지난 한 달", "금일", "일주일","지금까지의")를 `WHERE` 절에 반영하세요.  
                    ...
                    - 예: 특정 제품명이 `'임펠러'`인 데이터를 필터링하려면 `WHERE name = '임펠러'` 조건을 추가하세요. 다른 제품도 마찬가지로 제품명('기어A', '기어B', '엔질벨트A', '엔진벨트B', '엔진벨브', '임펠러')을 넣어서 결과를 보여줘야 합니다. 

                    5. **예시 쿼리 조건**:  
                    -  불량률 제품별 계산 가능해야해. - `smwp.quality_management_tb`
                    - **생산률(진척률)**을 제품별로 계산 가능해야해. - `smwp.생산`
                    - `smwp.quality_management_tb`와 `smwp.생산` 테이블을 조인.  
